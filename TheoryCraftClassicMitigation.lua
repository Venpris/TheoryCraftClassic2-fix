...
local _, spellname, targetname, damage, crit, armor, i, spelldata, idx
...
local function ArmorFromReduction(reduction, level)
	reduction = math.max(0, math.min(0.75, reduction or 0))
	return math.floor((85 * level + 400) * reduction / (1 - reduction))
end

function TheoryCraft_ParseCombat(spellID, spellname, targetname, damage, crit, absorbed)
	...
	ul = UnitLevel("player")
	if (spellID == nil) or (spellname == nil) or (targetname == nil)
	   or (damage == nil) or (tonumber(damage) == nil) then return end
	...
	spelldata = TheoryCraft_GenerateSpellData(spellID)
	if armor then
		...
		if isplayer then
			local hadarmor = TheoryCraft_MitigationPlayers[targetname]
				and TheoryCraft_MitigationPlayers[targetname][1] ~= nil
			if not hadarmor then
				oldarmorvalue = 0
				for i = 0,60 do
			...
			if not hadarmor then
				if (damage > 0) and (damage < spelldata.maxdamage+1) then
					...
					TheoryCraft_MitigationPlayers[targetname][1] = ArmorFromReduction(damagereduction, ul)
		...
		-- Not a player
			...
			if not TheoryCraft_MitigationMobs[targetname] then
				if (damage > 0) and (damage < spelldata.maxdamage+1) then
					...
					TheoryCraft_MitigationMobs[targetname][1] = ArmorFromReduction(damagereduction, ul)
