...
--]]
	...
	local spellName, spellID = game_tooltip_frame:GetSpell()
	...
	-- Tooltip post-calls can repeat for the same data after lines are added.
	-- Mark the spell before adding TheoryCraft lines to avoid duplicates.
	if game_tooltip_frame.TheoryCraftSpellID == spellID then return end

	local tooltipdata = TheoryCraft_GetSpellDataByID(spellID)

	if tooltipdata == nil then return end
	game_tooltip_frame.TheoryCraftSpellID = spellID
