...
local function TheoryCraft_AddEquipEffect(slotname, test, data, equippedsets)
	...
	local rtext = getglobal("TheoryCraftTooltipTextRight"..index)
	if rtext then
	...
	local startpos, endpos, start, found, foundme
	while (ltext ~= nil) do
...
function TheoryCraft_CombineTables(tab1, tab2)
	if tab2 == nil then tab2 = {} end
	for k,v in pairs(tab1) do
		if k == "procs" and type(v) == "table" then
			-- Proc entries describe independent effects. Appending them keeps
			-- different items from being numerically merged into one record.
			tab2[k] = tab2[k] or {}
			for _, proc in ipairs(v) do
				table.insert(tab2[k], TheoryCraft_CombineTables(proc, {}))
			end
		elseif type(v) == "table" then
			tab2[k] = TheoryCraft_CombineTables(v, tab2[k])
		elseif type(v) == "number" and type(tab2[k] or 0) == "number" then
			tab2[k] = v + (tab2[k] or 0)
		elseif tab2[k] == nil then
			-- Preserve string/boolean metadata such as a proc's target stat.
			tab2[k] = v
		end
	end
	return tab2
...
local function TheoryCraft_AddAllEquips(outfit, force)
	...
	if not TheoryCraft_Data.EquippedSets then
