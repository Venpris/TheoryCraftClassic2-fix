...
TheoryCraft_Data.CalculationsReady = false
...
function TheoryCraft_OnEvent(self, event, ...)
	...
	if event == "ADDON_LOADED" then
		...
		-- Prefer the post-processing API introduced with the modern tooltip
		-- system. Fall back for older Classic clients.
		if GameTooltip.HasScript and GameTooltip:HasScript("OnTooltipSetSpell") then
		   ...
		   and Enum and Enum.TooltipDataType and Enum.TooltipDataType.Spell then
			TooltipDataProcessor.AddTooltipPostCall(Enum.TooltipDataType.Spell, function(tooltip)
				if tooltip == GameTooltip then
					TheoryCraft_AddTooltipInfo(tooltip)
				end
			end)
		else
		...
		GameTooltip:HookScript("OnHide", function(tooltip)
			tooltip.TheoryCraftSpellID = nil
		end)
	...
	elseif event == "PLAYER_LOGIN" then
		...
		C_Timer.After(2, function()
			...
			TheoryCraft_Data.CalculationsReady =
				TheoryCraft_Data.Stats["totalmana"] ~= nil
	...
	elseif event == "COMBAT_LOG_EVENT_UNFILTERED" then
		local _, subevent, _, sourceGUID, _, _, _, _, destName, _, _,
			spellID, spellName, _, amount, _, _, _, _, absorbed, critical =
			CombatLogGetCurrentEventInfo()
		if sourceGUID == UnitGUID("player")
		   and (subevent == "SPELL_DAMAGE" or subevent == "RANGE_DAMAGE")
		   and spellID and spellName and amount then
			if Ambiguate and destName then
				destName = Ambiguate(destName, "none")
			end
			TheoryCraft_ParseCombat(spellID, spellName, destName, amount, critical, absorbed)
		end
	...
	-- Spell names, ranks, and descriptions can arrive asynchronously. Rebuild
	-- cached calculations after the spellbook or its text becomes available.
	elseif event == "SPELLS_CHANGED" or event == "SPELL_TEXT_UPDATE" then
		TheoryCraft_DeleteTable(TheoryCraft_TooltipData)
		TheoryCraft_DeleteTable(TheoryCraft_UpdatedButtons)
		TheoryCraft_UpdateAllButtonText('spell data changed')
	...
	elseif event == "UNIT_POWER_UPDATE" then
		if arg[1] == "player" then
			if TCUtils.StanceFormName('UNIT_POWER_UPDATE') == 'cat' then
				TheoryCraft_DeleteTable(TheoryCraft_UpdatedButtons)
			end
			-- These values depend on the player's current resource pool.
			if ((string.find(TheoryCraft_Settings["tryfirst"] or "", "remaining")) or
			    (string.find(TheoryCraft_Settings["trysecond"] or "", "remaining"))) or
			   ((TheoryCraft_Settings["tryfirst"] == "spellcasts") or
			    (TheoryCraft_Settings["trysecond"] == "spellcasts")) then
				TheoryCraft_DeleteTable(TheoryCraft_UpdatedButtons)
				TheoryCraft_UpdateAllButtonText('power changed')
			end
		end
