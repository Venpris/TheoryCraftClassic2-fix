...
-- Classic Era 1.15.9 moved spellbook updates from the removed global
-- SpellBookFrame_UpdateSpells function to methods on SpellBookFrame.
if SpellBookFrame and type(SpellBookFrame.Update) == "function" then
	hooksecurefunc(SpellBookFrame, "Update", UpdateSpellBookText)
elseif type(SpellBookFrame_UpdateSpells) == "function" then
	hooksecurefunc("SpellBookFrame_UpdateSpells", UpdateSpellBookText)
end
...
function TheoryCraft_ButtonUpdate(this, actionbar_slot_changed)
	...
	if buttontext.type == "SpellBook" then
		...
		if id then
			...
			local subSpellName = _G[this:GetName().."SubSpellName"]
			local rank = subSpellName and subSpellName:GetText() or ""
