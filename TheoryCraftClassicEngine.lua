	...
	while (spellname) do
		...
		elseif (found == 1) then
			TCTooltip:SetOwner(UIParent,"ANCHOR_NONE")
				TheoryCraft_SetTooltipSpellBookItem(TCTooltip, i2-1, BOOKTYPE_SPELL)
...
local function TheoryCraft_GetRangedSpeed()
	...
	local index = 1
	local ltext = getglobal(TheoryCraftTooltip:GetName().."TextLeft"..index)
	local rtext = getglobal(TheoryCraftTooltip:GetName().."TextRight"..index)
	local _, start, found
	if ltext then
		ltext = ltext:GetText()
	end
...
local function GetCritChance(critreport)
	...
	TheoryCraftTooltip:SetOwner(UIParent,"ANCHOR_NONE")
	TheoryCraft_SetTooltipSpellBookItem(TheoryCraftTooltip, id, BOOKTYPE_SPELL)
...
local function AddProcs(casttime, returndata, spelldata)
	...
	if not TheoryCraft_Settings["procs"] then return end
	...
	if TheoryCraft_Data.EquipEffects["procs"] then
		...
		while TheoryCraft_Data.EquipEffects["procs"][i] do
			local procData = TheoryCraft_Data.EquipEffects["procs"][i]
			amount = procData.amount
			if amount and procData.proc and procData.duration and procData.type then
				if procData.exact == true or procData.exact == 1 then
					procduration = 1 - (1 - procData.proc)^math.floor(procData.duration/casttime)
				else
					procduration = 1 - (1 - procData.proc)^(procData.duration/casttime)
				end
				TheoryCraft_Data.Procs[procData.type] =
					(TheoryCraft_Data.Procs[procData.type] or 0) + amount * procduration
			end
			i = i + 1
		end
		i = 1
...
local function GenerateTooltip(frame, returndata, spelldata, spellrank)
	...
	returndata["casttime"] = returndata["basecasttimeseconds"] or
		tonumber(returndata["basecasttime"]) or 0
	...
	local spelllevel = 60
	local spellpercent = spelldata.percent
	...
	if (spelldata["level"..spellrank.."per"])      then spellpercent = spelldata["level"..spellrank.."per"] end
	...
	if (spelllevel < 20) then
		spellpercent = spellpercent * (0.0375 * spelllevel + 0.25)
	end
	...
	if (spelldata.ismelee == nil) and (spelldata.isranged == nil) then
		...
		if (spelldata.isheal == nil) and (spelldata.talentsbeforegear == nil) then
			returndata["damcoef"] = spellpercent * returndata["tmpincrease"] * returndata["tmpincreaseupfront"]
		else
			returndata["damcoef"] = spellpercent
		end
		...
		if spelldata.righteousness and TheoryCraft_Data.EquipEffects["MeleeAPMult"] >= 3 then
			...
			returndata["damcoef"] = spelldata.percent2 * returndata["tmpincrease"]
		end

		-- if there is a dot component
		if (spelldata.hasdot == 1) then
			returndata["damcoef2"] = spelldata.percentdot
			returndata["plusdam2"] = math.floor(returndata["plusdam"] * returndata["damcoef2"])
		end
		...
		if (returndata["mindamage"] and returndata["maxdamage"] and (spelldata.isseal == nil)) and (spelldata.block == nil) then 
			...
			if spelldata.isranged and (spelldata.shoot == nil) then
				if spelldata.autoshot then
					...
					returndata["maxoomdam"]     = (TheoryCraft_GetStat("totalmana") + TheoryCraft_GetStat("manarestore")) * returndata["dpm"]
	...
	-- Not a seal, melee or ranged ability (eg, regular spell)
		...
		if returndata["minheal"] then
			...
			if spelldata.dontdomax == nil and returndata["hpm"] then
				returndata["maxoomheal"]     = (TheoryCraft_GetStat("totalmana") + TheoryCraft_GetStat("manarestore")) * returndata["hpm"]
	...
	if returndata["manacost"] then
		returndata["maxspellcasts"] = math.max(0,
			1 + math.floor((TheoryCraft_GetStat("totalmana") -
				(returndata["basemanacost"] or returndata["manacost"])) /
				returndata["manacost"]))
...
function TheoryCraft_GenerateSpellData(spellId)
	...
	-- Action-bar addons can request spell data between PLAYER_LOGIN and the
	-- delayed gear/stat initialization. Returning no data briefly is safer
	-- than calculating from a partially populated Stats table.
	if not TheoryCraft_Data.CalculationsReady then return nil end
	...
	local spellDescription = GetSpellDescription(spellId) or ""
	local spellRank = GetSpellRankById(spellId) or 0
	...
	local olddesc = spellDescription
	...
	TheoryCraft_TooltipData[spellName.."("..spellRank..")"] = spellId
		...
		if (frame) and (frame ~= TCTooltip2) then
			...
			if (spellrank == nil) then
				...
				while (spellname == spellname2) do
					...
					if spellrank == nil then spellrank = 0 end
					TCTooltip:SetOwner(UIParent,"ANCHOR_NONE")
					TheoryCraft_SetTooltipSpellBookItem(TCTooltip, i2, BOOKTYPE_SPELL)
			...
			TheoryCraft_SetTooltipSpellBookItem(frame, i2, BOOKTYPE_SPELL)
			if frame:NumLines() == 0 then return end
		end
	end
...
local function UpdateTarget(data)
	...
	if data["armor"] and TheoryCraft_Settings["mitigation"] then
		...
		if data["msrotationdps"] then data["msrotationdps"] = data["msrotationdps"]*armormult end
...
function TheoryCraft_GetSpellDataByID(spellId)
...
function TheoryCraft_GetSpellDataByAction(action_slot)
	...
	return TheoryCraft_GetSpellDataByID(id)
end

function TheoryCraft_GetSpellDataByName(name, rank)
	local wantedRank = tonumber(rank) or 0
	local spellId
	local index = 1
	while true do
		local spellName, spellSubtext, candidateID = GetSpellBookItemName(index, BOOKTYPE_SPELL)
		if not spellName then break end
		local candidateRank = tonumber(TCUtils.findpattern(spellSubtext or "", "%d+")) or 0
		if spellName == name and candidateRank == wantedRank then
			spellId = candidateID
			break
		end
		index = index + 1
	...
	return TheoryCraft_GetSpellDataByID(spellId)
end
...
function GetSpellRankById(spellId)
	...
	local rank = string.match(spellSubtext, "%d+")
...
function TheoryCraft_GetSpellDataByDescription(description, force)
	...
	while (true) do
		...
		if spellrank == nil then spellrank = 0 end
		TCTooltip:SetOwner(UIParent,"ANCHOR_NONE")
		TheoryCraft_SetTooltipSpellBookItem(TCTooltip, i, BOOKTYPE_SPELL)
